...
fun agoToText(secAgo: Int) {
    when {
        secAgo in 0..60 -> println("был(а) в сети только что")
        secAgo > 60 && secAgo <= 60 * 60 -> println("был(а) в сети ${secAgo / 60} ${secToText(secAgo)} назад")
        secAgo > 60 * 60 && secAgo < 24 * 60 * 60 -> println("был(а) в сети ${secAgo / (60 * 60)} ${hourToText(secAgo)} назад")
...
fun secToText(secAgo: Int): String {
    val min = secAgo / 60
    val toText = when {
...
fun hourToText(secAgo: Int): String {
    val hour = secAgo / (60 * 60)
    val toText = when {
